...
class LNWallet(LNWorker):
    ...
    def __init__(self, wallet: 'Abstract_Wallet', xprv):
        ...
        self.pending_htlcs = defaultdict(asyncio.Queue)  # type: Dict[bytes, asyncio.Future[BarePaymentAttemptLog]]
        self.mpp_htlcs = defaultdict(set) # type: Dict[bytes, set]
    ...
    async def _pay(
        ...
        while True:
            ...
            # 3. await a queue
            payment_attempt = await self.pending_htlcs[payment_hash].get()
            # if we get a tmp failure, it might work to split the amount and try more routes
            # if we get a channel update, we might retry the same route and amount
            # we also need a criterion for giving up
            if payment_attempt.success:
                amount_received += payment_attempt.amount_msat
                if amount_received >= amount_to_pay:
                    self.payment_sent(payment_hash)
                    success = True
                    log.append(PaymentAttemptLog(success=True))
                    break
                else:
                    continue
            # failure
            if payment_attempt.failure_message:
                failure_msg = payment_attempt.failure_message
                sender_idx = payment_attempt.sender_idx
                ...
                if code == OnionFailureCode.MPP_TIMEOUT:
                    ...
                    mpp_timeout = True
                    break
                elif code == OnionFailureCode.TEMPORARY_CHANNEL_FAILURE:
                    # add a max to the channel capacity
                    self.temp_channel_failures[short_chan_id] = payment_attempt.amount_msat
            ...
            failure_log = PaymentAttemptFailureDetails(
                sender_idx=sender_idx,
                failure_msg=failure_msg,
                is_blacklisted=False)
            payment_attempt_log = PaymentAttemptLog(
    ...
    async def pay_to_route(self, route: LNPaymentRoute, amount_msat:int, lnaddr: LnAddr) -> PaymentAttemptLog:
        # send a single htlc
    ...
    def handle_error_code_from_failed_htlc(self, failure_msg, sender_idx, route):
        code, data = failure_msg.code, failure_msg.data
    ...
    def create_routes_from_invoice(
        ...
        # TODO: MPP: return multiples routes if we know that a single one will not work
    ...
    def add_received_htlc(self, htlc, expected_msat):
        status = self.get_payment_status(htlc.payment_hash)
        if status == PR_PAID:
            return True, None
        s = self.mpp_htlcs[htlc.payment_hash]
        if htlc not in s:
            s.add(htlc)
        total = sum([htlc.amount_msat for htlc in s])
        first_timestamp = min([htlc.timestamp for htlc in s])
    ...
    async def await_payment(self, payment_hash: bytes) -> PaymentAttemptLog:
    ...
    def htlc_fulfilled(self, chan, payment_hash: bytes, amount_msat:int):
        htlc_log = HtlcLog(
            success=True,
            amount_msat=amount_msat)
        q = self.pending_htlcs[payment_hash]
    ...
    def htlc_failed(
        ...
        if error_bytes and route:
            ...
            is_blacklisted = self.handle_error_code_from_failed_htlc(failure_message, sender_idx, route)
            if is_blacklisted:
    ...
    def payment_failed(
        ...
        if f and not f.cancelled():
            payment_attempt = PaymentAttemptLog(
                success=False,
                error_bytes=error_bytes,
                failure_message=failure_message)
